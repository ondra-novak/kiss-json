...
template<typename Number, typename Fn>
void floatToString(Number value, Fn &&fn, int maxPrecisionDigits=9) {
     static std::uint32_t fracMultTable[10] = {
                    1, //0
                    10, //1
                    100, //2
                    1000, //3
     ...
     const char *inf = "∞";
          ...
          bool sign = value < 0;
          std::uint32_t precisz = std::min<std::uint32_t>(maxPrecisionDigits, 9);
          ...
          std::uint32_t fractMultiply = fracMultTable[precisz];
