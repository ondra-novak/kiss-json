...
#ifndef KISSJSON_BASE64_H_
#define KISSJSON_BASE64_H_
...
template<typename Fn>
void base64decode(const std::string_view &binary, Fn &&output, const Base64Table &table = Base64Table::get_default_table()) {
    auto iter = binary.begin();
    auto iend = binary.end();
    int b=0,c=0,d=0,e=0;
